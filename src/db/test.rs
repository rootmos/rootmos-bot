...
pub fn put_and_get_key_test<K: Eq + Debug, V: Eq + Debug, T: db::KV<K, V>>(mut db: T, k: K, v: V) {
    assert_eq!(db.get(&k), Ok(None));
    assert_eq!(db.put(&k, &v), Ok(()));
    assert_eq!(db.get(&k), Ok(Some(v)))
}

pub fn overwrite_key_test<K: Eq + Debug, V: Eq + Debug, T: db::KV<K, V>>(mut db: T, k: K, v1: V, v2: V) {
...
pub fn remove_key_test<K: Eq + Debug, V: Eq + Debug, T: db::KV<K, V>>(mut db: T, k: K, v: V) {
    assert_eq!(db.put(&k, &v), Ok(()));
    assert_eq!(db.get(&k), Ok(Some(v)));
...
pub fn fetch_keys_by_prefix_test<K, V, DB, KGen, PKGen, VGen>(mut db: DB, prefix: K, non_prefixed_key:  KGen, prefixed_key: PKGen, value: VGen) -> ()
